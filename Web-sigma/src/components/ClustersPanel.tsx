                ...
                <>
                    <MdGroupWork className="text-muted" /> Clusters
                        <span className="text-muted text-small">
                            {" "}
                        ({visibleClustersCount} / {clusters.length} clusters)
                        </span>
                </>
            }
        >
            <p>
                <i className="text-muted">Click a cluster to show/hide the channels in that community from the network.</i>
            </p>
            <p className="buttons">
                ...
                {sortedClusters.map((cluster) => {
                    ...
                    return (
                        <li
                            ...
                            key={cluster.key}
                            title={`${nodesCount} channel${nodesCount > 1 ? "s" : ""}${visibleNodesCount !== nodesCount ? ` (only ${visibleNodesCount} visible)` : ""
                                }`}
                        >
                            <input
                                type="checkbox"
